...
class WeekData {
	...
	public static function reloadWeekFiles(isStoryMode:Null<Bool> = false, mv:Bool = false, clear:Bool = true)
	{
		if (clear)
		{
			weeksList = [];
		...
		if (mv)
			sexList = CoolUtil.coolTextFile(Paths.getPreloadPath('mvweeks/weekList.txt'));
		for (i in 0...sexList.length) {
			for (j in 0...directories.length) {
				var fileToCheck:String = directories[j] + 'weeks/' + sexList[i] + '.json';
				if (mv)
					fileToCheck = directories[j] + 'mvweeks/' + sexList[i] + '.json';
		...
		for (i in 0...directories.length) {
			...
			if (mv)
				directory = directories[i] + 'mvweeks/';
